...
<p>최초에는 블럭들의 합의 평균값에 근사해지는 값을 1씩 더하거나 뺄때 시간이 짧은쪽을 선택해가며 답을 도출하려했으나 시간제한에 막혔다. 답은 정확하게 나오나 시간제한을 해결하기 위해 브루트포스 알고리즘을 선택해야했다. 입력조건에서 땅의높이가 256으로 주어졌으니 이를 이용해 브루트포스 알고리즘을 사용했다.</p>
